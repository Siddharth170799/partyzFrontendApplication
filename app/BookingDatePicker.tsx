...
const DatePicker = ({ posting }) => {
  ...
  const [getFunctionHallName,setFunctionHallName] = useState(null)
  ...
  const bookingDatesData = async () => {
    const details = await axios.get(
      "http://192.168.0.6:4000/api/getBookingDateDetails"
    );
    ...
    const details2 = details.data.map((item) => {
      return item?.Date;
    });
    ...
    const details3 = details1.map((item)=>{
      return item.FunctionHall
    })
    setFunctionHallName(details3);
  ...
  return (
    <>
      ...
      <BookingPage date={selectedDate} bookingDates={bookingDates2}  functionHallName={getFunctionHallName}  />
    </>
  );
};
