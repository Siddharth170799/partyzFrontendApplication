...
const DatePicker = ({posting}) => {
  ...
  console.log(selectedHallName,"jjj")
  ...
  const bookingDatesData = async () => {
    const details = await axios.get(
      "http://192.168.0.5:4000/api/getBookingDateDetails"
    );
  ...
  const markedDates = bookingDates?.map((item)=> item.Date).reduce((acc, date) => {
    acc[date] = { marked: true, dotColor: "red", disabled: true };

    return acc;
  }, {});
  console.log(bookingDates,"sidd")
